...
| 规范来源 | 本文定义稳定产品契约；生产运行状态以 [运维与交接手册](HANDOFF_WEB_UPGRADE.md) 为准 |
...
| 导出模板 | 第 2 行为正式 A:G 表头，第 3 行保留样例格式 | 约束导出字段和样式 |
...
多文件合并 Excel 按批次来源顺序拼接可导入数据；分文件 ZIP 为每份来源文件保留独立工作簿；单文件批次继续支持直接下载对应结果。
...
超收规则迁移是幂等迁移，可重复执行。完整的发布前检查、回退边界和故障处理见 [运维与交接手册](HANDOFF_WEB_UPGRADE.md)。
...
`acceptance_data/` 只用于本机验收，不得提交到 Git。当前实际测试与部署证据不写入 README，统一记录在 [验收报告](design-qa.md) 和 [运维与交接手册](HANDOFF_WEB_UPGRADE.md)。
...
| 文档 | 用途 | 主要读者 |
| --- | --- | --- |
| [运维与交接手册](HANDOFF_WEB_UPGRADE.md) | 生产基线、部署、验证、故障检查、数据安全与接续开发 | 维护与部署人员 |
| [PC 界面规范](UI_UX_OPTIMIZATION_PLAN.md) | 信息架构、页面契约、交互安全和桌面端验收要求 | 产品、设计与前端维护人员 |
| [PC 界面验收报告](design-qa.md) | 需求到证据的验收矩阵、自动化结果与残余风险 | 发布复核人员 |
