...
当前代码门槛已通过自动化验证；Linux 目标机已从 `feature/admin-maintenance` 工作树完成 Compose 重建，数据库和 API 健康，Worker 与 Web 正常运行，Web 健康接口返回 `{"status":"ok"}`，登录后的版本列表、库位摘要/预览、用户和操作记录只读接口均成功，新前端资源哈希已生效。管理员维护 PC 端 Chrome 验收已通过，过程中发现的发布警告弹窗首屏操作不可达问题已经修复并复验。现网没有编辑中的库位草稿，因此写流程使用隔离 QA/临时 SQLite 数据库验收，没有向生产库写入测试草稿。完整脱敏业务场景以及数据库/文件卷成对备份恢复等运维门槛仍需完成，完成前不要宣称生产可用。
