...
2026-07-21 在 `feature/admin-maintenance` 工作树完成的当前基线是 Python 102/102 通过、前端 6 个测试文件共 53/53 通过、`pip check` 无冲突，生产构建成功。构建仍有约 1.2 MB 单包体积提示，不影响当前功能。
...
当前代码门槛已通过自动化验证；Linux 目标机已从 `feature/admin-maintenance` 工作树完成 Compose 重建，数据库和 API 健康，Worker 与 Web 正常运行，Web 健康接口返回 `{"status":"ok"}`，登录后的版本列表、库位摘要/预览、用户和操作记录只读接口均成功，新前端资源哈希已生效。现网没有编辑中的库位草稿，因此写流程使用临时 SQLite 数据库验收，没有向生产库写入测试草稿。完整脱敏业务场景以及数据库/文件卷成对备份恢复等运维门槛仍需完成，完成前不要宣称生产可用。
